...
let request = requestLib
    .defaults({
        ...
        headers: {
            ...
            'Accept-Language': 'zh-TW,zh;q=0.9,zh-CN;q=0.8,en-US;q=0.7,en;q=0.6,ko;q=0.5,ja;q=0.4,th;q=0.3',
...
/**
 * Get csrf token
 * @return {String} - return csrf token
 * @api private
 */
const getCsrfToken = co.wrap(function* () {
    logger.info('get csrftoken...')
    const res = yield request.get('/');
    const csrfTokenCookie = cookie.parse(res[0].headers['set-cookie'][1]);
    const csrfToken = csrfTokenCookie['csrftoken'];
    logger.info(`csrfToken: ${csrfToken}`)
    if (!csrfToken) {
        throw new Error('cannot get csrftoken, please help to create an issue for author.')
    }
    return csrfToken
})
 ...
 * @param {Array} cookies - the cookies in response header
 * @return {String} - return LEETCODE_SESSION
 * @api private
 */
const getLeetcodeSession = co.wrap(function* (cookies) {
    logger.info('get LEETCODE_SESSION...')
    const cookieObject = cookie.parse(cookies[2]);
    const leetcodeSession = cookieObject['LEETCODE_SESSION'];
    logger.info(`LEETCODE_SESSION: ${leetcodeSession}`)
    if (!leetcodeSession) {
        throw new Error('cannot get LEETCODE_SESSION, please help to create an issue for author.')
    }
 ...
 * @api public
 */
const login = co.wrap(function* (conf) {
    config = conf
    ...
    const leetcodeSession = yield getLeetcodeSession(res[0].headers['set-cookie'])
    if (leetcodeSession) {
